...
>
> 状态：**OPEN（R5 修订版）**——§1 已按授权落 Packet v4/v5；§2 三项中 **A' 已撤回（否决记录在案）**、**A 定稿版** 维持待重审（本轮不动，仅交叉引用更新）、**B 升 v3**——**停在裁定点：待 Maintainer 裁定"输入域收窄"**（不只是待重审，见 §2-B 裁定清单）；含 XAUDIT-FIX 与 R4 两轮勘正（§4）
>
> 关联：DR-IP-0022-01/-02/-03；Packet `IP-0022-PACKET/v5`（R5 修订）；`LIMA_IP-0022_XAUDIT_Matrices_v1.md`（R4+R5 修订）；Assignment `IP-0022-PV-XAUDIT/v1`、`IP-0022-PV-R4/v1`、`IP-0022-PV-R5/v1`（PKT-IP-0022-R4 / R5，2026-09-13）
>
...
### DR-04-B：公共 API 往返契约——入口域收窄 + candidate_id 一致性 —— **v3（停在裁定点：待 Maintainer 裁定输入域收窄）**
...
- **推荐方案（入口域收窄 + 类型化拒绝，Maintainer 已给框架、本版定稿）**：
...
- **兼容性与所需冻结面裁定（必须显式裁定，呈 Maintainer）**：
  - **输入域收窄 = 可观察行为变更**：IP-0019 `build_semantic_top_n` 与 IP-0021 `ram_wire_payload` 两个**冻结公开入口**，此前接受的 `symbol='-'` 输入此后被 typed 拒绝（该输入形态本身是 #58 契约合法的）；
  - **正常扫描产物零影响**：五源定源在案（P-R4-3 维持）——默认扫描链永不产出 ranked `symbol='-'`；golden fixtures 与 real-chain payload 零命中（v4/v5 复核维持）；
  - **所需裁定 1（门控）**：授权对上述两个冻结公开入口执行输入域收窄（规则 1/2）。不获批则维持 R-9 残余风险（None↔`'-'` 篡改不可检出）；
  - **备选方案 B-2（不推荐，如实呈报不暗改）**：若 Maintainer 不收窄输入域，唯一替代是改 candidate_id 冻结编码消歧（如 `'-'` 转义或换 None 哨兵）= **IP-0019 冻结面重冻结**（golden 期望值与 collision 断言全量变更，大 DR），本 DR 不请求该变更，仅列为备选供裁定时对照。
  - **所需裁定 2（随裁定 1 一并）**：wire 层第七项消歧 + 一致性公式（v2 设计维持：prefix 匹配 + `rsplit("#",1)` 得 symbol 槽与非负数字 ordinal 等值比对，违例 `$.semantic.ranked[i].candidate_id`）。
- **测试映射（_red_proof 底稿已入库，RED D1''''' = 39 failed / 4 passed）**：X8-0 真实扫描全链往返 GREEN 锚（基线即绿）；X8-1 `build_semantic_top_n` 入口 '-' typed 拒绝（RED：main 接受，往返通过）；X8-2 `ram_wire_payload` 直构结果入口 '-' typed 拒绝（RED：main 接受）；X4-7 wire 层 None→`'-'` 篡改必拒（RED 维持，注释已明确与入口负例分工）；X4-1/2/3 三字段一致性绑定（RED 维持，无退化）；X4-0 锚（IP-0021 冻结 candidate_id 模式，含非数字 ordinal）不受影响。
- **不批的影响**：wire 的 kind/path/symbol 三字段保持"仅结构校验 + path 规则"绑定，含 None↔`'-'` 互换在内的篡改不可检出（R-9 残余风险，矩阵二）；且 `'-'` 合法构造输入的往返缺陷维持现状（能生成能校验但语义歧义）。
...
## 4. 验证证据（摘要，全文见 Packet v4 §9 / v5 §9.5-9.6 与矩阵文档）

- RED D1'''''（R5 增量）：`_red_proof` **39 failed / 4 passed**（R4 的 37 failed / 3 passed + 新文件 `test_ip_0022_r5_red.py`：X8-1/X8-2 两例 RED——两个公开入口当前**接受** symbol='-'，`assertRaises(ContractError)` 未触发即目标缺失型签名——+ X8-0 真实扫描全链往返 GREEN 锚通过）。回归锚 `tests/audit tests/contracts` 801 passed、golden 15 passed（R5 本轮工作树亲跑）。
...
请 Coordinator/Maintainer 对 §2 **A（定稿版：`manifest-index` 稳定标识 + 2 文件 4 断言锚定修订清单，本轮不动，维持 R4 呈报）** 与 **B（v3：入口域收窄 + 类型化拒绝 + wire 消歧/一致性公式）** 作出裁定（A' 已撤回，不再呈请）。**B 的裁定点为"输入域收窄"**（对 IP-0019 `build_semantic_top_n` 与 IP-0021 `ram_wire_payload` 两个冻结公开入口拒绝 `symbol='-'` 合法形态）：获批则并入 Packet v5 对应条目 mandatory 集（当前标【DR-04-A/B】）；不收窄且不接受 R-9 残余 ⇒ 须转 B-2（IP-0019 重冻结，另行大 DR）；否则维持残余风险记账（R-1 / R-9）。§1 各项（含 v5 script 名称准入）已按 Assignment 授权落 Packet，无需另行裁定。
