...
- Packet 版本：**1.7**（2026-09-18，P&V erratum rev.6，状态 DESIGN-FROZEN-REVISED / PENDING-MERGE。**v1.6 → v1.7 修订**：按 Maintainer 收口性五次修订指令 **R17**（仅两项、不得新增范围；已入 Issue #94 Ledger 决策日志 2026-09-18；R14/R15/R16 与三项呈报裁定确认不得改写）：**R13 errno 契约修正**——逐组件链维持 `os.open(comp, O_RDONLY|O_DIRECTORY|O_NOFOLLOW, dir_fd=parent_fd)`；组合标志下目录 symlink 组件 open 在部分平台返回 `ENOTDIR` 而非 `ELOOP`（**Maintainer Linux 容器实测在案**）：`ELOOP` 保持 → `target-invalid:symlink-component`（部分平台仍返回 ELOOP）；`ENOTDIR` 新增**一次诊断步骤** `os.stat(component, dir_fd=parent_fd, follow_symlinks=False)`——`S_ISLNK` → `target-invalid:symlink-component`、确非目录 → `target-invalid:not-a-directory`、诊断异常/路径替换 → `target-invalid:io-error`；**任何分支一律 fail-closed（禁跟随/重试/读取内容）**；类别枚举与链结构不变（§17.F5'-R13 第 3 条改写、第 5 条联动措辞）；除 R13 契约与本版本记录外零改动。此前 v1.6：**v1.5 → v1.6 修订**：按 Maintainer 四项四次修订指令 R13-R16（已入 Issue #94 Ledger 决策日志 2026-09-13）：R13 逐组件 openat 链获取目录句柄（绝对锚 `/`、相对锚 `.`；逐组件 `os.open(comp, O_RDONLY|O_DIRECTORY|O_NOFOLLOW, dir_fd=parent_fd)` 每步关父 fd；ELOOP → exit 2 + `target-invalid:symlink-component`、ENOTDIR → `target-invalid:not-a-directory`；v1.5 步骤 0 "lstat→按路径 open→fstat↔lstat 比对"序列 [v1.5-replaced] 整体废止，比对步骤删除不留冗余）（§17.F5'-R13、§17.F5'-R2.1'' 步骤 0、§17.B2'）；R14 选择集后缀先筛（仅支持后缀 `.json`/`.txt` 计入 top-K，其他目录项静默忽略——不计 10,000 配额、不占工件序号、不产生 warning、不入 incomplete_reasons；file-budget 触发条件改述"支持后缀条目数 > 10,000"）（§17.F5'-R9 第 1/4 条）；R15 报告级 findings 总量上限 `max_report_findings = 10_000`（脚本层常量，**脚本层聚合处实施、库签名零改动**——`build_audit_report` 冻结面不动；逐文件累计达上限即停 + 恰一条 `scan:findings-budget:truncated-at=a<n>` + stderr 单行汇总 + status incomplete + exit 3，报告仍产出且 findings 恰为 10,000）（§17.F5'-R15）；R16 目录替换测试五项最低要求（①正向读取证明：原目录 ≥2 哨兵 S1/S2 各产出 findings——零读取实现不得通过；②外部哨兵零出现；③返回结构完整 artifact_count==2；④启动边界 R13 联动；⑤中间链接场景；seam 生产同路径 grep 锁）（§18 #25/#30）。**三项呈报裁定落实**：①R9 措辞勘误——"名称收集遍 O(N) 目录项读取、无硬时限承诺"（不得称硬时间上限）[Maintainer-accepted 2026-09-13，措辞勘误落实]；②R10 Windows fail-closed（exit 2 + `platform-unsupported-safe-scan`，Windows 用户需 WSL/POSIX）[Maintainer-accepted 2026-09-13]；③R11 seam 接受，条件 = 测试经生产同一路径并证明实际读取行为（原目录哨兵 findings 产出 + seam 被主流程调用 grep 锁）（§17.B2'-R10 seam 段、§18 #25/#30）。**8 处旧条款冲突清理**（修订非删除，逐处 [v1.5-replaced] 或勘误）：§17.F5'-R2.1'' 步骤 0 / §17.B2' 读取句（按路径 open 活跃冲突）/ §17.B2'-R3 门 B 保留段内部指针 / §17.B2' 目录-symlink stderr 串 / §17.B2' 遍历段 `os.scandir(target)` → `os.scandir(dir_fd)` / §17.F5'-R9 第 1 条（后缀先筛）/ §17.F5'-R9 第 5 条（触发条件）/ 本 Header v1.5 段 O(N) 措辞。开工基线前移至 origin/main = `dfa0f85a92b14bbbc46ea9f465c381d953f2a797`（IP-0022 #180，与 #94 零文件交集）；本分支 rebase 至新基线（零冲突），commit 映射：`74ef696→effb59a`、`ecea634→9099290`、`00c4939→9be7b6a`、`333e376→5049e77`。此前 v1.5：按 Maintainer 四项三次修订指令 R9-R12（已入 Issue #94 Ledger 决策日志 2026-09-13）：R9 枚举规则定稿方案 (i)——单遍枚举 + 有界 top-K 字典序选择（内存 O(10,000)，枚举遍仅读 dirent 零打开；"立即停止枚举"改写为"停止打开/处理"；`a<n>` = 选择集内字典序 0 基；N>10,000 → 恰一条 `scan:file-budget:truncated-at=a9999`；名称收集遍 O(N) 目录项读取、无硬时限承诺 [v1.6 勘误：原"残余 O(N) 名称收集时间界如实声明"措辞废止，见 §17.F5'-R9 第 2 条]）（§17.F5'-R9）；R10 目录句柄绑定 dir_fd（R2.1' 四步序列升级为 R2.1'' **五步版**；按路径 open 形态废止；启动 lstat→`os.open(target, O_RDONLY|O_DIRECTORY)`→fstat↔lstat (st_dev, st_ino) 比对闭合目录打开窗口；逐文件 `os.open(name, O_RDONLY|O_NOFOLLOW|O_NONBLOCK, dir_fd=dir_fd)` 相对名；Windows 分支定稿 fail-closed——exit 2 + stderr 单行 `platform-unsupported-safe-scan`、不产出报告；v1.4 Windows 回退条款 [v1.4-replaced]）（§17.B2'-R10、§17.F5'-R2.1''）；R11 特殊文件（O_NONBLOCK 打开 + `S_ISREG` 类型门 → special-file 逐文件跳过，条目数受选择集 ≤10,000 上界约束明文写入）（§17.F5'-R11）；R12 勘误（§18 #18 行 "第 10,001 起逐文件" 表述与 R8/#23 矛盾，统一到 R9 选择集语义，[v1.4-erratum]）。**随新 head 呈报 Maintainer 三项**：①R9 残余 O(N) 枚举时间界（只读 dirent 零打开；硬时限须另立 DR 引入时间预算维度）[Maintainer-accepted 2026-09-13，措辞勘误落实——"名称收集遍 O(N) 目录项读取、无硬时限承诺"]；②R10 Windows 产品语义（脚本 Windows 不可运行 exit 2，Windows 用户需 WSL/POSIX 环境）；③R11 `_read_artifacts(dir_fd, names, ...)` 模块级私有 seam 授权口径。此前 v1.4：开工基线前移至 origin/main = `30bdfaa13ac72572d65ccb2567ae8702923c4187`（IP-0021 #179，与 #94 零文件交集）；本分支 rebase 至新基线（零冲突），commit 映射：`493caeb→333e376`、`eab3365→00c4939`。**v1.3 → v1.4 修订**：按 Maintainer 三项二次修订指令 R6-R8（已入 Issue #94 Ledger 决策日志 2026-09-13）：R6 source_path 唯一合法形态收紧为 `^a(0|[1-9][0-9]{0,3})$`（a0–a9999，与 `max_files_per_scan=10_000` 一一对应封闭；字符白名单任何变体已证伪并废止，§17.D1'.3、§18 #20）；R7 读取统一为四步单一序列——`os.open`（POSIX O_NOFOLLOW）→ `fstat(fd)`↔`entry.stat(follow_symlinks=False)` 比对 → **同一 fd** 经 `os.fdopen` 执行 `read(LIMIT+1)` → with 关闭；禁 `open(path,"rb")`/重开/`read_bytes()`/`st_size` 预检（§17.F5'-R2.1'、§17.B2'-R3、§18 #22）；R8 预算触顶**即停**：`incomplete_reasons` 恰一条 `"scan:file-budget:truncated-at=a{n>}"`（字节同型）、stderr 单行汇总、非逐文件条目，报告规模有界（§17.F5'-R2.2'、§18 #23）。此前 v1.3：按 Maintainer 对 PR #178 的五项修订指令 R1-R5（已入 Issue #94 Ledger 决策日志 2026-09-13；规格增量正本 = COORD-IP0020-R1R5_SPEC_ADDENDUM_2026-09-13，P&V 逐项落实）：R1 `TenantAuditContext` repr=False + 自定义 `__repr__` 仅含 tenant_id_len/tenant_key_len，构造异常链全查零 key（§17.D2'-R1、§18 #17）；R2 句柄限量读取 `f.read(LIMIT+1)`（禁 `read_bytes()`/st_size 预检）+ 目录级 `max_files_per_scan=10_000`/`max_total_bytes_per_scan=104_857_600`（§17.F5'-R2、§18 #18）；R3 定稿机制 A——`os.open`（POSIX 叠加 O_NOFOLLOW）+ `fstat(fd)` 与 `entry.stat(follow_symlinks=False)` 比对 `(st_dev, st_ino)`，不一致或 st_ino==0 一律 fail-closed（symlink-risk），v1.2 "Windows 注明残余 TOCTOU" 条款废止（§17.B2'-R3、§18 #19）；R4 库侧 source_path 白名单 `^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$` → `INVALID_FIELD_VALUE("source_path")`（新增 §17.D1'.3、§18 #20）；R5 derived_key 定稿（HMAC-SHA256(tenant_key, b"lima.evidence_privacy.tenant-key.v1\x00"+tenant_id.utf8)，对齐 fingerprint.py 实物）+ `.pv_tmp` 权威引用替换为 §17 自含 + Ledger 锚点（§17.D2'-R5、§18 #21、R5.3 替换清单）。**顺序裁定**：预备 Frozen v3（commit 2a47a1e，分支 codex/ip-0020-frozen-tests-v3）仅为预备 RED 证据；正式 Frozen v3-final 于本 Packet 合并后在合并 main 上重建并重证 RED。此前 v1.2：**v1.1 → v1.2 修订（erratum，修订非删除）**：按 Maintainer 修订裁定 **DR-IP-0020-4R**（裁定正本 = Issue #94 Ledger 决策日志 2026-09-13（Maintainer DR-IP-0020-4R 修订裁定，稳定仓库锚点）；契约增量规格 = 本 Packet §17（自含权威）；erratum commit = 本修订合并 commit SHA（合并时回填）。裁定精神：暂不整组批准 DR-IP-0020-4，六项修订执行——D1' 位置模型 v2（成员序号路径 + 全携带面零原值）、D2' `TenantAuditContext` 聚合入口租户绑定（混合 fail-closed、内部长标签双重防漏出、零 finding 批次覆盖）、D3' 脚本 per-run 随机密钥（禁固定密钥回流）、F4' vault 错误 backend 元数据化、B2' symlink 边界 + Linux 权威负例、F5' 预算分层 + `status`/`incomplete_reasons` + exit 3；附随 F6' Ruff 清零、G3' `classify_payload` 空租户 fail-closed）。修订落点：§8.1.3/§8.1.5（F4'）、§8.2（D1'/D2'/F5' 库层）、§8.3（D1'.2/D3'/B2'/F5' 脚本层）、§9 T2/T4（携带面与租户隔离升级）、§11/§12（断言面与验收命令）、新增 **§17 v1.2 契约增量规格（逐小节）与 §18 断言映射总表（修订非删除，逐条"旧 → 新"映射）**。被 v1.2 替代的 v1.1 条款在本节内以 **[v1.2-replaced]** 标注保留原文（修订非删除），冲突时以 §17/§18 为准。既有 8 文件零修改边界、`__all__` 13 项、`PrivacyErrorCode` 12 值锚定不动（红线 5；`audit.py` 扩 `TenantAuditContext` 属 DR-IP-0020-1 后置授权情形，DR-IP-0020-4R 即授权，见 §17.D2'）。此前：v1.1（2026-09-13，P&V 起草，状态 DESIGN-FROZEN / PENDING-MERGE；v1.0 同日发布。v1.0 → v1.1 修订：按 Coordinator PR #173 评审裁定 COORD-IP0020-PACKET_PR_REVIEW 2026-09-13 + AI Reviewer 提前检查 REVIEW-IP-0020-SECBOUNDARY_EARLYCHECK 发现项 **F-1（MINOR）**——§8.2.3 指纹需租户凭据 vs §8.3 脚本 CLI 无租户参数 vs §9 T4 表述的租户凭据未定义缺口，采用"固定离线租户"方案修订：新增 §8.3.8、改写 §9 T4、更新 §11.1 断言组、§11.5 补 Windows 路径变体用例（非阻塞建议采纳）。其余内容零改动）
...
> (e) 遍历改 `os.scandir`；`entry.is_symlink()` → 跳过 + `{"artifact": "a<n>", "error": "symlink-skipped"}` + `incomplete_reasons` 记录；读取采用 R3 定稿机制 A（`os.open` + POSIX `O_NOFOLLOW` + `fstat(fd)`/`entry.stat(follow_symlinks=False)` 的 `(st_dev, st_ino)` 比对，不一致或 `st_ino==0` 一律 fail-closed `symlink-risk`；v1.2 "Windows 注明残余 TOCTOU" 条款废止，见 §17.B2'-R3）；`target_dir` 本身或中间目录解析后为 symlink → 拒绝 exit 2（**[v1.5-replaced]** 原 stderr 串 `error: target-dir must not be a symlink path` 废止，v1.6 统一为 `target-invalid:symlink-component`（§17.F5'-R13 §1.2 第 5 条语义）；调用方传入路径允许回显）。
...
- **[v1.4-replaced 原文，R7 四步序列]**："(1) 按路径 `os.open(path, O_RDONLY|O_NOFOLLOW)` → (2) fstat↔entry.stat 比对 → (3) fdopen 限量读 → (4) with 关闭"——按路径 open 形态被 R10 dir_fd 绑定废止，fstat 比对被五步序列第 2 步吸收。
- **§17.F5'-R2.1''（R10+R11 定稿，v1.5 权威，唯一规范五步序列）**：
  1. **步骤 0（每扫描一次）**：启动序列——**[v1.5-replaced 原文，R13 废止]** "`lstat(target_dir)` 非 `S_ISLNK`（既有 exit 2 条款不变）→ `dir_fd = os.open(target_dir, O_RDONLY|O_DIRECTORY)` → `fstat(dir_fd)` 与 lstat 比对 `(st_dev, st_ino)`（一致且 `st_ino != 0` 通过；不一致 → exit 2，stderr `error: target-dir validation failed (dev/ino mismatch)`——闭合"lstat 与 open 之间目录被替换"窗口；O_DIRECTORY 不隐含 O_NOFOLLOW，已核）"——该 lstat + 按路径 open + fstat↔lstat 比对序列整体废止（比对只证明"打开的目录 == lstat 看到的目录"，不证明路径树上无中间链接；单次按路径 open 全路径解析会跟随中间目录 symlink），被 **§17.F5'-R13 逐组件 openat 链**替代（fstat↔lstat 比对步骤**删除**，不留防御性冗余——冗余路径上的不一致无规范处置语义）；平台能力前置探测（`os.supports_dir_fd` 含 `os.open`、`hasattr(os, "O_NOFOLLOW")`、`hasattr(os, "O_NONBLOCK")`）**保留且顺序最先**，任一缺失 → **exit 2 + stderr 单行 `platform-unsupported-safe-scan`，不产出报告**（Windows 必然落入此分支，fail-closed 定稿，不降级按路径打开；先于组件链执行）；名称收集遍用 `os.scandir(dir_fd)`（绑定同句柄，R9）。
  2. **步骤 1（打开）**：`fd = os.open(name, O_RDONLY|O_NOFOLLOW|O_NONBLOCK, dir_fd=dir_fd)`——`name` 为 scandir 返回的**相对名**，禁路径拼接打开（每文件恰一次 `os.open`，继承 R7 计数断言）。
  3. **步骤 2（单一 fstat 双门）**：`st_fd = os.fstat(fd)`（全序列仅此一次 fstat，同用于两门）：门 A（类型，R11）`not stat.S_ISREG(st_fd.st_mode)` → 关闭 fd → `special-file` 跳过（§17.F5'-R11）；门 B（链接/竞态，R3 语义继承）`(st_fd.st_dev, st_fd.st_ino)` 与 `entry.stat(follow_symlinks=False)` 比对不一致或 `st_ino == 0` → 关闭 fd → `symlink-risk` fail-closed（v1.4 语义不变）。
  4. **步骤 3（限量读）**：同一 fd 经 `os.fdopen(fd, "rb")`（接管所有权）`read(PrivacyLimits().max_payload_bytes + 1)`；超限 → resource-limit 单文件跳过（R2 语义不变；普通文件不受 O_NONBLOCK 影响，照常读取）。
  5. **步骤 4（关闭）**：with 块结束唯一关闭路径；dir_fd 扫描结束后关闭。**禁止项（继承 R7 全部 + R10/R11 新增）**：按路径 `open(`、`path.read_bytes()`、`st_size` 预检、二次打开、`os.path.join` 构造被打开路径（join 仍允许用于报告/日志路径）；grep 断言在 R7 #22 基础上扩 `dir_fd=`/`O_NONBLOCK`/`S_ISREG`/零按路径拼前缀打开；行为测试锁定继承 R7 计数断言（每文件至多一次 `os.open`、零 `builtins.open`，含跳过路径），§18 行 #22/#25。以句柄实际读取字节数判长（`len(data) > max_payload_bytes` → 该文件可识别跳过：stderr `{"artifact": "a<n>", "error": "resource-limit"}`；`incomplete_reasons` 增 `"a<n>:resource-limit"`）。**禁止** `path.read_bytes()` 全量读后判长（main 现状 scripts/audit_sensitive_artifacts.py:67 即此形态，RED 点）；禁止以 `stat().st_size` 预检替代（stat 可被竞态替换，FIFO/proc 类 st_size 无意义）。上限取值经 `PrivacyLimits()` 实例（= 1_048_576，实物 @ models.py），不硬编码字面量。
- **目录级预算（R2 定稿，仅脚本层，不入 `PrivacyLimits`/库签名）**：`max_files_per_scan = 10_000`（镜像 `max_items` 基数上界）；`max_total_bytes_per_scan = 104_857_600`（= max_payload_bytes × 100，聚合上界 100 MiB）。**[v1.3-replaced 原文，R8 废止]** "任一预算触顶后后续文件逐个按 resource-limit 可识别跳过（同型 stderr + `a<n>:resource-limit`；聚合面另记一条目录级原因）"——逐项语义废止（极大目录下扫描时间与报告大小无界）。**§17.F5'-R2.2'（R8 定稿，R9 语义重构）触顶即停 + 有界输出**：**[v1.4-replaced]** "扫描枚举达到 10,000 后立即停止枚举与逐项处理"——"立即停止枚举"废止（字典序确定性要求名称收集遍全遍），由 **§17.F5'-R9 选择集语义**替代：枚举遍不可中断但仅选择集内文件被打开/处理；文件预算——名称收集遍结束时支持后缀条目数 N' > 10,000（R14 后缀先筛限定）→ 处理完选择集后，`incomplete_reasons` 追加**恰好一条** `"scan:file-budget:truncated-at=a9999"`（恒为选择集末位工件号 a9999，定长有界；stderr 单行同串汇总）；字节预算同理——**选择集处理过程中**累计读取字节超 104,857,600 后停止处理选择集内后续文件，追加恰好一条 `"scan:byte-budget:truncated-at=a{n}"`（n = 停止时最后已处理工件号；计数口径继承 v1.3/v1.4：跳过文件按 LIMIT+1 计入）；stderr 截断时输出**单行**汇总警示（同上字符串格式），非逐文件；`status:"incomplete"` + exit 3 不变；报告规模因此有界（findings ≤ 已处理文件产出，reasons ≤ 事件常数）；不得 exit 1/2、不得静默；单文件超限 resource-limit 语义（R2.1' 第 3 步）不变。断言见 §18 行 #18/#23。
...
### 17.F5'-R9 枚举规则定稿：单遍枚举 + 有界 top-K 字典序选择（R9，Maintainer 指令 2026-09-13）

1. **名称收集遍（仅 dirent，零打开；R14 后缀先筛修订）**：`os.scandir(dir_fd)`（dir_fd 绑定见 R10/R13）单遍遍历；对每个 entry 仅取 `entry.name`，不调用 `entry.stat()`、不 `open` 任何文件、不读任何字节；**仅支持后缀 `.json` / `.txt`（大小写敏感，按后缀精确匹配，与 §8.3 原契约"遍历 `*.json`/`*.txt`"一致）的目录项计入**容量 10,000 的有界选择结构维护字典序最小子集（堆或等价有序结构，实现自选；内存 O(10,000)；被淘汰文件零读取、零 IO 副作用）；**不支持后缀的目录项静默忽略——不计入 10,000 配额、不占工件序号、不产生任何 warning、不入 `incomplete_reasons`**（§8.3 契约范围外，"范围外"≠"未完成"）。后缀判定用 `str.endswith`/`PurePath.suffix` 于 scandir 返回的相对名，不做任何 open/stat（保持名称收集遍零打开）。**[v1.5-replaced]** v1.5 本条"对每个 entry 计入 top-K（无后缀过滤）"表述废止。symlink 条目同样仅收集名（仅限支持后缀），跳过判定延迟到选择集处理阶段（继承 B2' "symlink 占工件序号"语义）。
2. **"立即停止"语义改写**：枚举遍不可中断（字典序确定性要求全遍；scandir 返回序为文件系统实现细节，非字典序不可移植）；停止的对象是**打开/处理**——仅选择集内 ≤ 10,000 个文件被打开、读取、产出 findings。扫描时间 O(N·log K) ≈ O(N)（K=10,000 常数）；**名称收集遍历为 O(N) 目录项读取（只读 dirent、零打开），无硬时限承诺**（磁盘枚举耗时为审计固有成本，不打开文件已消除主要风险面；硬时限须另立 DR 引入时间预算维度）。**[v1.6 勘误]** v1.5 "残余 O(N) 名称收集时间界如实声明"措辞废止（[Maintainer-accepted 2026-09-13，措辞勘误落实]——不得称硬时间上限）。
3. **`a<n>` 编号 = 后缀过滤后选择集内字典序 0 基**：选择集按名升序排序后第 i（0 基）个文件 = `a{i}`（i ≤ 9,999，与 R6 形态封闭一致）。选择集内 symlink 按其字典序位号跳过并占号，不重排后续编号。
4. **文件预算（R8 #23 语义重构；R14 触发条件改述）**：选择集构造上限即 `max_files_per_scan = 10,000`，逐文件计数式触顶在结构上不再发生；名称收集遍结束时若**支持后缀条目数 N' > 10,000** → 处理完选择集后 `incomplete_reasons` 追加恰好一条 `"scan:file-budget:truncated-at=a9999"`（stderr 单行同串汇总）、`status:"incomplete"`、exit 3；N' ≤ 10,000 → 无此条目，预算路径完全沉默。**[v1.5-replaced]** v1.5 本条"条目总数 N > 10,000"（无后缀限定）表述废止。
5. **报告规模界** = 选择集 ≤ 10,000 个文件产出（reasons 以选择集为界）；findings 总量另受 **§17.F5'-R15 `max_report_findings = 10_000`** 报告级上限封顶。

### 17.B2'-R10 目录句柄绑定与平台 fail-closed（R10，Maintainer 指令 2026-09-13）

- **启动序列（一次性）**与**逐文件打开唯一形态**（相对名 + `dir_fd=`，禁路径拼接打开）见 §17.F5'-R2.1'' 步骤 0/1；`os.open(name, flags, dir_fd=fd)` 相对 `fd` 指向的目录 inode 解析 `name`，不受路径树上 rename/symlink 替换影响。
- **平台分支（fail-closed 定稿）**：前置能力探测（`os.supports_dir_fd` 含 `os.open`、`O_NOFOLLOW`、`O_NONBLOCK`）任一不支持 → 脚本拒绝运行：stderr 一行 `error: platform-unsupported-safe-scan`，exit 2，不产出报告文件；不降级为按路径打开。产品影响：Windows 用户需 WSL/POSIX 环境执行审计（呈报 Maintainer 确认）。
- **可测缝（seam，R10 行为负例依赖）**：模块级私有函数 `_read_artifacts(dir_fd: int, names: list[str], ...) -> ...`（接收已验证 dir_fd 与相对名列表，执行五步序列；主流程与测试共用，不新增公开 API；seam 存在及其被主流程调用均由 grep 锁定，防"测试专用后门"）。授权口径：属 IP-0020 脚本层内部重构，按 DR-IP-0020-4R "脚本层修订授权"理解（**[Maintainer-accepted 2026-09-13]**，条件：测试经 seam 的断言须证明实际读取行为（原目录哨兵 findings 产出）且 seam 被主流程调用（grep 锁）——见 §17.F5'-R16 与 §18 #25/#30）。

### 17.F5'-R11 特殊文件（FIFO/设备等）（R11，Maintainer 指令 2026-09-13）

1. 所有文件统一 `O_RDONLY | O_NOFOLLOW | O_NONBLOCK` 打开（并入 R2.1'' 步骤 1）：O_NONBLOCK 使 open(2) 对 FIFO（无写端 O_RDONLY 阻塞）与设备文件立即返回，消除"fstat/预算前阻塞在 open"；对普通文件 open/read 无影响。
2. 打开后单一 fstat 门 A（类型）：`not stat.S_ISREG(st_fd.st_mode)`（FIFO/S_IFCHR/S_IFBLK/S_IFSOCK 等）→ 关闭 fd、跳过、stderr 单行 `{"artifact": "a<n>", "error": "special-file"}`、`incomplete_reasons` 增 `"a<n>:special-file"`、`status:"incomplete"`、exit 3（O_NONBLOCK 保证 open 已返回，fstat 不再可能阻塞）。
3. **special-file 计数的界（明文）**：逐文件记一条，不并入截断汇总，但受 R9 选择集上限约束——special-file 条目至多 10,000 条，报告规模仍以选择集为界（≤ 10,000 × 事件常数，闭合非无界；逐文件可识别跳过优先于进一步压缩，与 symlink-risk/resource-limit 粒度一致）。
4. `special-file` 为新增 stderr/incomplete_reasons 字符串类目（不动 `PrivacyErrorCode` 12 值冻结）；并入"总断言"：任何 special-file 出现 ⇒ status != complete 且 exit == 3（§18 #19 扩类目）。

### 17.F5'-R13 逐组件 openat 链获取目录句柄（R13，Maintainer 指令 2026-09-13；中间目录符号链接闭合）

1. **组件解析（不触文件系统）**：`target_dir`（CLI 传入原样字符串）经 `os.path.split`/`PurePath` 逻辑拆为组件序列——不做 `resolve()`（resolve 会跟随链接，禁）。锚点裁定：**绝对路径 → 锚点 `fd = os.open("/", O_RDONLY | O_DIRECTORY)`**（根锚不可能是 symlink，openat 链从根起每步 `dir_fd=` 即可全链防替换，不依赖 AT_FDCWD）；**相对路径 → 锚点 `fd = os.open(".", O_RDONLY | O_DIRECTORY)`**（CWD 锚用 `"."` 获取 fd 后，后续组件全部经 `dir_fd=` 相对解析；`.` 本身非 symlink——进程 CWD 是 inode 引用非路径；**不使用 AT_FDCWD 逐组件方案**：CWD 在链中漂移会破坏链一致性，锚定 fd 一次性固化）。Windows 盘符/反斜杠路径：平台能力探测（R10 前置，顺序最先）在 Windows 必然 fail-closed exit 2，组件拆分只需覆盖 POSIX 形态（含 `./`、`../`；`..` 组件按普通目录组件 openat 解析，锚定 fd 链下相对父 fd 解析，不回退路径树语义）。锚点 open 的平台弹性：若 POSIX 实测 `os.open(".", O_DIRECTORY|O_NOFOLLOW)` 返回 EINVAL（个别系统怪癖），允许锚点 open 去掉 O_NOFOLLOW（锚点非 symlink 论证同上）；**组件链上 O_NOFOLLOW 不得去**。
2. **逐组件链**：对每个组件 `comp`：`next_fd = os.open(comp, os.O_RDONLY | os.O_DIRECTORY | os.O_NOFOLLOW, dir_fd=parent_fd)`，成功后 `os.close(parent_fd)`（每步关闭父 fd，任意时刻至多 2 个链上 fd；链完成后最终 fd 即 `dir_fd`，交接 R2.1'' 步骤 1——R9 名称收集遍 `os.scandir(dir_fd)` 照旧）。
3. **错误分类（类别枚举冻结，stderr 单行，exit 2；v1.7/R17 errno 契约修正）**：链上组件 open 失败按 errno 分类，`ENOTDIR` 经一次诊断步骤定类——`ELOOP` → `target-invalid:symlink-component`（部分平台对目录 symlink 组合 open 仍返回 ELOOP）；`ENOTDIR` → **一次诊断步骤** `os.stat(component, dir_fd=parent_fd, follow_symlinks=False)`：`stat.S_ISLNK(st_mode)` → `target-invalid:symlink-component`（**Maintainer Linux 容器实测依据：组合标志 `O_RDONLY|O_DIRECTORY|O_NOFOLLOW` 下目录 symlink 组件 open 返回 `ENOTDIR` 而非 `ELOOP`**，symlink 判定经此诊断闭合）、确非目录（非 symlink 的非目录项）→ `target-invalid:not-a-directory`、诊断自身 `OSError`（含诊断时路径已被替换）→ `target-invalid:io-error`；`EACCES`/`EPERM` → `target-invalid:permission-denied`；`ENOENT` → `target-invalid:not-found`；其他 `OSError` → `target-invalid:io-error`（兜底）。**诊断与分类约束：仅此一次 `os.stat`（`follow_symlinks=False`，不跟随、不读取内容），任何分支一律 fail-closed——禁跟随链接、禁重试 open、禁读取组件内容。**类别枚举 = `{symlink-component, not-a-directory, permission-denied, not-found, io-error}`（不变）；stderr 允许回显调用方传入路径（参数错误口径不变），但不回显组件原文之外任何路径树信息。
4. **语义闭合声明**：锚定 fd 起每步 openat 相对父 fd 解析，`O_NOFOLLOW` 逐步拒绝链接——任一组件为 symlink 或链中任一目录被 rename/替换，链要么打开原始 inode（后续步骤不受路径树变化影响）要么报错；"中间目录链接拒绝"由此兑现，不依赖"打开后比对"。v1.5 步骤 0 的 fstat↔lstat (st_dev, st_ino) 比对步骤**删除**（链本身即闭合"中间链接 + 打开窗口替换"两个面；事后 lstat 路径比对反而重新引入路径树假阴性/假阳性）；B2' 门 B（逐文件 fstat↔entry.stat 比对）不受影响（文件级最终组件竞态门，保留）。
5. **§17.B2' "目录本身为 symlink" 条款对齐**：末组件（target_dir 本身）为 symlink 同样由链上 `O_NOFOLLOW` 拒绝——`ELOOP`、或 `ENOTDIR` 经第 3 条 v1.7 诊断步骤，均 → `target-invalid:symlink-component`（旧 stderr 串 `error: target-dir must not be a symlink path` [v1.5-replaced] 废止，统一进 `target-invalid:` 枚举；非链接的 target-invalid 类别 exit 2 语义继承既有参数错误口径）。

### 17.F5'-R14 选择集后缀先筛（R14，Maintainer 指令 2026-09-13）

规格已并入 **§17.F5'-R9 第 1/3/4 条**（仅支持后缀 `.json`/`.txt` 计入 top-K；其他目录项静默忽略，不计配额、不占号、无 warning、不入 incomplete；`a<n>` = 后缀过滤后选择集字典序 0 基；file-budget 触发条件 = 支持后缀条目数 > 10,000；后缀判定零打开）。负例见 §18 #28。

### 17.F5'-R15 报告级 findings 总量上限（R15，Maintainer 指令 2026-09-13）

1. **上限定稿**：`max_report_findings = 10_000`（脚本层常量，命名与 `max_files_per_scan` 同族；与单次调用上限同值——"文件数 ≤10,000 × 单文件 findings 无界"的乘积面被同一常数封顶，无需第二量纲）。
2. **实施层（脚本层聚合处，库签名零改动）**：`build_audit_report`（lima/evidence_privacy/audit.py，`__all__` 冻结面）不动；脚本在逐文件处理循环中累计 findings 计数，达 10,000 即**停止处理后续文件**（与 R8 字节预算截断同型：不继续打开/读取选择集内剩余文件）：`incomplete_reasons` 追加**恰好一条** `"scan:findings-budget:truncated-at=a<n>"`（n = 停止时最后已处理工件号）；stderr 单行汇总（同串）；`status:"incomplete"` + exit 3；报告仍成功产出（findings 恰为上限 10,000 条，已累计部分全保留）；与 file-budget/byte-budget 截断标记可共存（各自恰一条，互不吞并；截断顺序按触发即停的运行时事实记录）。负例见 §18 #29。

### 17.F5'-R16 目录替换测试五项最低要求（R16，Maintainer 指令 2026-09-13；承载于 §18 #25 修订行）

1. **正向证明实际读取**：原目录内 ≥2 个普通文件（各含互异已知哨兵 S1/S2，后缀 .txt/.json 各一更佳）→ rename+symlink 替换后调 `_read_artifacts` → 断言二者各产出 findings（S1/S2 对应指纹/位置各非空）——零读取实现（不读任何文件直接返回空）不得通过。
2. **外部目录哨兵零出现**：替换用外部目录含敏感哨兵 X 及其文件 → X 与外部文件任何内容在任何返回值/报告零出现。
3. **返回结构完整**：`artifact_count == 2`、报告对象非 None、结构 schema 合法。
4. **启动边界（R13 联动）**：目录替换发生在打开前/链中 → `target-invalid:*` exit 2（R13 负例组覆盖）或链绑定原目录（链终 fd 语义）——二选一断言，不得"路径树已换仍静默扫描新目录"。
5. **中间链接场景**：同 R13 负例 1，并入 #25 断言组。
- **生产路径要求（R11 呈报条件）**：测试对 seam 的调用必须与脚本主流程同一路径——grep 锁主流程（`main`/`_scan_artifacts` 等价层）**实际调用** `_read_artifacts`（调用点存在，非仅定义），禁测试专用旁路；seam 断言的是实际读取行为（S1/S2 findings 产出），非仅"返回不崩溃"。



- **遍历**：`os.scandir(dir_fd)`（**[v1.6 勘误]** v1.5 原文 "`os.scandir(target)`"（按路径）废止——与 R9/R10/R13 启动序列统一为 dir_fd 绑定）；`entry.is_symlink()` → 跳过 + stderr `{"artifact": "a<n>", "error": "symlink-skipped"}`（symlink 亦占工件序号，保证序号稳定）+ `incomplete_reasons` 记录。不跟随目录内 symlink（无论指向文件或目录）。
- **[v1.5-replaced 原文，R13/R10 废止]** "读取：POSIX 用 `os.open(path, os.O_RDONLY | os.O_NOFOLLOW)` 后 `os.read` 防 TOCTOU（O_NOFOLLOW 只挡最终组件 symlink）"——按路径 open 形态废止，统一按 **§17.F5'-R2.1'' 步骤 1**（相对名 + `dir_fd=`）执行。
- **[v1.2-replaced 原文，R3 废止]** "Windows 回退：先 `entry.is_symlink()` 检查再 open；注明残余 TOCTOU 窗口，以 Linux CI 负例为权威"——该条款废止。**R3 定稿（机制 A；R10 升级为机制 A2，按路径 `os.open(path, flags)` 形态 [v1.4-replaced] 废止）**：统一按 **§17.F5'-R2.1'' 五步序列**执行（dir_fd 绑定 + 单一 fstat 双门）；门 B 语义原文（保留）：`fst = os.fstat(fd)` 与遍历时取得的 `est = entry.stat(follow_symlinks=False)` 比对 `(st_dev, st_ino)`：一致且 `est.st_ino != 0` → 按 **§17.F5'-R2.1'' 五步序列步骤 3**（**[v1.6 勘误]** 原内部指针 "R2.1' 四步序列" 过时改指；语义不变）以同一 fd 经 `os.fdopen` 限量读取（**[v1.3-replaced]** 原"以同一 fd 做 R2.1 句柄限量读取"分立表述统一并入 R2.1'，禁任何二次打开）；否则 **fail closed**——关闭 fd、跳过、stderr `{"artifact": "a<n>", "error": "symlink-risk"}`、`incomplete_reasons` 增 `"a<n>:symlink-risk"`、`status:"incomplete"`（该文件零内容进入任何输出）；"验证不可行"（Windows `st_ino == 0`）与"验证失败"同权，均不读取、不宣称完成。`symlink-risk` 为新增 stderr/`incomplete_reasons` 字符串类目（不动 `PrivacyErrorCode` 12 值冻结）。**[v1.4-replaced]** "Windows `st_ino` 可用性为实现期验证项（实测典型 NTFS 路径 st_ino != 0；普遍为 0 须提交 DR）"——该 Windows 回退条款整体废止：Windows 不再有"条件 flags + st_ino 可用性实测"路径，统一走 R10 平台 fail-closed（exit 2 `platform-unsupported-safe-scan`）。总断言：任何 `symlink-risk`/`symlink-skipped`/`resource-limit`/`unparseable-json`/`undecodable-text` 出现 ⇒ `status != "complete"` 且 exit == 3（§18 行 #19）。
- **目录本身为 symlink**（target_dir 或中间目录解析后为链接）→ 拒绝：**[v1.5-replaced]** 原 stderr 串 `error: target-dir must not be a symlink path` + exit 2 废止——v1.6 统一由 §17.F5'-R13 逐组件链 `O_NOFOLLOW` → `ELOOP` → **exit 2 + stderr 单行 `target-invalid:symlink-component`**（参数错误，允许回显调用方传入路径；非链接类 target-invalid 类别同 exit 2，见 R13 §3 枚举）。
...
| 18 | 脚本 `read_bytes()` 全量读（scripts/audit_sensitive_artifacts.py:67，无对应断言） | 新增：1 MiB+1B → resource-limit 跳过（句柄读取形态，源码 grep 无 `read_bytes()`，对 c5b375e RED）；恰 1 MiB 正常读（边界含）；**[v1.4-erratum，R12]** "10_001 文件目录 → 第 10_001 起 resource-limit + `scan:file-budget`"表述废止（与 R8/#23 触顶即停矛盾），统一到 R9 选择集语义：10,005 文件目录 → 选择集 = 字典序最小 10,000（a0–a9999）、被淘汰 5 文件零贡献（哨兵零出现代理）、`incomplete_reasons` 恰 1 条 `scan:file-budget:truncated-at=a9999`、stderr 单行汇总、无逐文件 a10000+ 条目、incomplete + exit 3（§17.F5'-R9）；合计 >100 MiB → `scan:byte-budget` 同型恰 1 条（选择集处理中触发即停，truncated-at=a{n<9999} 变体）；预算触顶后报告仍成功产出 | R2+R12 勘误：v1.4 #18 行表述与 R8/#23 矛盾，统一到 R9 选择集语义 | R2/R9/R12 |
| 19 | §17.B2' v1.2 草案"Windows 注明残余 TOCTOU"表述（main 无对应实现断言） | 改/新增：`os.open`+`os.fstat`+`(st_dev,st_ino)` 比对机制（源码 grep `os.fstat`/`st_ino`）；注入 stat 不一致 → 跳过 + `symlink-risk` + incomplete + 内容零泄漏；`st_ino==0` → fail closed；总断言：任何 symlink-risk/skipped/resource-limit/unparseable/undecodable/**special-file**（R11 扩类目）出现 ⇒ status != complete 且 exit==3；Windows 实测 st_ino 可用性为实现期验证项（§17.B2'-R3） | R3；B2' 链接边界意图继承，"注明竞态仍宣称完成"被 fail-closed 替代 | R3 |
...
| 22 | v1.3 #18/#19 R2 `open(path,"rb")` 与 R3 `os.open` 分立描述冲突 | 新增：同句柄锁定——grep 读取相关 `open(` 仅 `os.open(`/`os.fdopen(` 形态、无 `read_bytes`/`st_size` 预检（R10/R11 扩：`dir_fd=` 全覆盖、零按路径拼前缀打开、零 `os.path.join` 于打开路径、`O_NONBLOCK`/`S_ISREG` 锁；**R13 扩**：目录打开链 `O_DIRECTORY\|O_NOFOLLOW\|dir_fd=`、零单次按路径 `os.open(target…)`）；行为——wrap `builtins.open`/`os.open` 计数，每文件至多 1 次 `os.open`、0 次 `builtins.open`（含 symlink-risk 与超限/special-file 跳过路径）（§17.F5'-R2.1''） | R7+R10+R11+R13；竞态防护意图由分立条款统一为五步序列 + 逐组件链 | R7/R10/R11/R13 |
| 24 | （无——v1.4 及以前无选择集语义断言） | 修订（R14）：R9 选择集——10,005 个**支持后缀**文件（字典序最大 5 个含哨兵）→ 编号集合 {a0..a9999}、被淘汰文件零贡献（哨兵与对应编号零出现）、恰 1 条 `truncated-at=a9999`、stderr 单行、无逐项 a10000+ 条目、incomplete + exit 3（file-budget 触发条件 = **支持后缀**条目数 > 10,000）；字节预算 truncated-at=a{n<9999} 变体；混合后缀变体见 #28（§17.F5'-R9/R14） | R9+R14；R8 有界性意图由"停止枚举"重构为"有界选择集"；R9 O(N) 措辞勘误——名称收集遍 O(N) 目录项读取、无硬时限承诺 [Maintainer-accepted 2026-09-13] | R9/R14 |
| 25 | v1.4 按路径 `os.open(path, flags)` 形态（R3 机制 A） | 修订（R16，五项最低要求；[v1.5-replaced] 原"外部哨兵零出现"单断言）：grep（`dir_fd=` 全覆盖、零按路径拼前缀、`_read_artifacts` seam 被主流程调用，#30）；行为（POSIX/Linux 权威）目录替换竞态（rename + 原位 symlink→外部目录）五项断言——①**正向读取证明**：原目录 ≥2 哨兵 S1/S2 各产出 findings（零读取实现不得通过）；②外部哨兵 X 零出现零 findings；③`artifact_count == 2` + 报告结构完整；④启动边界（R13 联动：`target-invalid:*` exit 2 或链绑定原目录，二选一）；⑤中间链接场景（R13 负例 1 并入）；平台 fail-closed（Windows：exit 2 + `platform-unsupported-safe-scan` + 零报告产出，替代 v1.4 Windows 条件 skip）（§17.B2'-R10、§17.F5'-R13/R16） | R10+R16；竞态窗口闭合意图升级为目录 inode 绑定；R11 seam 呈报条件（实际读取证明 + 生产同路径）落实 | R10/R11/R13/R16 |
| 26 | （无——FIFO/设备文件无处理断言） | 新增：special-file——mkfifo（POSIX 条件）+ 超时包装 → 不阻塞、`{"artifact":"a<n>","error":"special-file"}` 警示、`incomplete_reasons` 含 `a<n>:special-file`、incomplete + exit 3、普通文件正常产出；grep 锁 `O_NONBLOCK`/`S_ISREG`（§17.F5'-R11） | R11；fail-closed 意图扩至特殊文件打开阻塞面 | R11 |
| 27 | （无——R13 前无逐组件链断言） | 新增（R13）：中间目录 symlink（`t/a` 链接→`t_real/a`，target=`t/a/b`）→ exit 2、stderr 含 `target-invalid:symlink-component`、零报告产出、b 内哨兵零读取零出现；锚点/相对语义组（cwd=target 下 `./` 与 `.` 调用 → 正常产出且 findings 与绝对形态等价；末组件 symlink → `target-invalid:symlink-component`；普通文件 target → `target-invalid:not-a-directory`）；grep：目录打开链 `O_DIRECTORY\|O_NOFOLLOW\|dir_fd=`、零单次按路径 `os.open(target…)`（对 main 基线 RED）（§17.F5'-R13） | R13；中间目录符号链接闭合意图由 v1.5 步骤 0 兑现 | R13 |
| 28 | （无——R14 前无后缀先筛断言） | 新增（R14）：混合后缀——10,000 个 `.bin`（字典序小于哨兵，如 `b00000.bin`…`b09999.bin`）+ 1 个含哨兵 `z-sentinel.txt` → 哨兵必被审计（findings 非空，哨兵指纹/位置产出）、`incomplete_reasons` 为空（无 file-budget 条目）、status complete、exit 0、`artifact_count == 1`、编号 `a0`、.bin 零打开零警告零序号（findings/source_path 零出现 + grep 锁）（§17.F5'-R14/R9） | R14；审计契约后缀范围外条目不得挤占选择集预算 | R14 |
| 29 | （无——R15 前无 findings 总量上限断言） | 新增（R15）：3 个 `.json` fixture 各含 4,000 个 base64 敏感值成员（3×4,000 = 12,000 > 10,000）→ 报告 findings 长度恰 10,000、`incomplete_reasons` 恰 1 条 `scan:findings-budget:truncated-at=a1`（a0/a1 各 4,000 + a2 前 2,000 后停止，按 `a<n>` 升序处理；精确 truncated-at 以定稿处理顺序为准）、stderr 恰一行 findings-budget 汇总无逐文件条目、exit 3 + status incomplete、报告 JSON 成功产出且规模有界（findings 数 == 上限为代理断言）（§17.F5'-R15） | R15；报告规模有界性封顶"文件数 × 单文件 findings"乘积面 | R15 |
| 30 | （无——R11 seam 呈报条件前无生产同路径 grep 断言） | 新增（R16/R11 呈报条件）：seam 生产同路径 grep——脚本主流程（`main`/`_scan_artifacts` 等价层）**实际调用** `_read_artifacts`（调用点存在，非仅定义）；测试禁专用旁路（不得另起直接读文件路径再比对）；seam 断言 = 实际读取行为（S1/S2 findings 产出，见 #25 ①） | R11 seam 接受条件落实 [Maintainer-accepted 2026-09-13]；防"测试专用后门" | R11/R16 |
...
**平台门控一致性（v1.6，#27-#29）**：#27-#29 行为断言 POSIX 跑；Windows 同用例断言 exit 2 + `platform-unsupported-safe-scan`（R10 fail-closed 全局先行，含 R13 组件链场景——平台探测先于组件链）；grep 类（#22/#30）全平台。
