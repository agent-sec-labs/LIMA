...
- **§17.F5'-R2.1''（R10+R11 定稿，v1.5 权威，唯一规范五步序列）**：
  1. **步骤 0（每扫描一次）**：启动序列——`lstat(target_dir)` 非 `S_ISLNK`（既有 exit 2 条款不变）→ `dir_fd = os.open(target_dir, O_RDONLY|O_DIRECTORY)` → `fstat(dir_fd)` 与 lstat 比对 `(st_dev, st_ino)`（一致且 `st_ino != 0` 通过；不一致 → exit 2，stderr `error: target-dir validation failed (dev/ino mismatch)`——闭合"lstat 与 open 之间目录被替换"窗口；O_DIRECTORY 不隐含 O_NOFOLLOW，已核）；平台能力前置探测（`os.supports_dir_fd` 含 `os.open`、`hasattr(os, "O_NOFOLLOW")`、`hasattr(os, "O_NONBLOCK")`）任一缺失 → **exit 2 + stderr 单行 `platform-unsupported-safe-scan`，不产出报告**（Windows 必然落入此分支，fail-closed 定稿，不降级按路径打开）；名称收集遍用 `os.scandir(dir_fd)`（绑定同句柄，R9）。
...
1. **名称收集遍（仅 dirent，零打开）**：`os.scandir(dir_fd)`（dir_fd 绑定见 R10）单遍遍历；对每个 entry 仅取 `entry.name`，不调用 `entry.stat()`、不 `open` 任何文件、不读任何字节；名集合经容量 10,000 的有界选择结构维护字典序最小子集（堆或等价有序结构，实现自选；内存 O(10,000)；被淘汰文件零读取、零 IO 副作用）。symlink 条目同样仅收集名，跳过判定延迟到选择集处理阶段（继承 B2' "symlink 占工件序号"语义）。
2. **"立即停止"语义改写**：枚举遍不可中断（字典序确定性要求全遍；scandir 返回序为文件系统实现细节，非字典序不可移植）；停止的对象是**打开/处理**——仅选择集内 ≤ 10,000 个文件被打开、读取、产出 findings。扫描时间 O(N·log K) ≈ O(N)（K=10,000 常数）；**残余 O(N) 名称收集时间界如实声明**（磁盘枚举为审计固有成本，不打开文件已消除主要风险面；硬时限须另立 DR 引入时间预算维度，呈报 Maintainer 确认）。
3. **`a<n>` 编号 = 选择集内字典序 0 基**：选择集按名升序排序后第 i（0 基）个文件 = `a{i}`（i ≤ 9,999，与 R6 形态封闭一致）。选择集内 symlink 按其字典序位号跳过并占号，不重排后续编号。
4. **文件预算（R8 #23 语义重构）**：选择集构造上限即 `max_files_per_scan = 10,000`，逐文件计数式触顶在结构上不再发生；名称收集遍结束时若条目总数 N > 10,000 → 处理完选择集后 `incomplete_reasons` 追加恰好一条 `"scan:file-budget:truncated-at=a9999"`（stderr 单行同串汇总）、`status:"incomplete"`、exit 3；N ≤ 10,000 → 无此条目，预算路径完全沉默。
5. **报告规模界** = 选择集 ≤ 10,000 个文件产出（findings/reasons 均以选择集为界）。
...
- **可测缝（seam，R10 行为负例依赖）**：模块级私有函数 `_read_artifacts(dir_fd: int, names: list[str], ...) -> ...`（接收已验证 dir_fd 与相对名列表，执行五步序列；主流程与测试共用，不新增公开 API；seam 存在及其被主流程调用均由 grep 锁定，防"测试专用后门"）。授权口径：属 IP-0020 脚本层内部重构，按 DR-IP-0020-4R "脚本层修订授权"理解（呈报 Maintainer 确认）。
...
| 22 | v1.3 #18/#19 R2 `open(path,"rb")` 与 R3 `os.open` 分立描述冲突 | 新增：同句柄锁定——grep 读取相关 `open(` 仅 `os.open(`/`os.fdopen(` 形态、无 `read_bytes`/`st_size` 预检（R10/R11 扩：`dir_fd=` 全覆盖、零按路径拼前缀打开、零 `os.path.join` 于打开路径、`O_NONBLOCK`/`S_ISREG` 锁）；行为——wrap `builtins.open`/`os.open` 计数，每文件至多 1 次 `os.open`、0 次 `builtins.open`（含 symlink-risk 与超限/special-file 跳过路径）（§17.F5'-R2.1''） | R7+R10+R11；竞态防护意图由分立条款统一为五步序列 | R7/R10/R11 |
| 24 | （无——v1.4 及以前无选择集语义断言） | 新增：R9 选择集——10,005 文件（字典序最大 5 个含哨兵）→ 编号集合 {a0..a9999}、被淘汰文件零贡献（哨兵与对应编号零出现）、恰 1 条 `truncated-at=a9999`、stderr 单行、无逐项 a10000+ 条目、incomplete + exit 3；字节预算 truncated-at=a{n<9999} 变体（§17.F5'-R9） | R9；R8 有界性意图由"停止枚举"重构为"有界选择集" | R9 |
| 25 | v1.4 按路径 `os.open(path, flags)` 形态（R3 机制 A） | 新增：dir_fd 绑定——grep（`dir_fd=` 全覆盖、零按路径拼前缀、`_read_artifacts` seam 被主流程调用）；行为（POSIX/Linux 权威）目录替换竞态（rename + 原位 symlink→外部目录，外部哨兵零出现零 findings）；平台 fail-closed（Windows：exit 2 + `platform-unsupported-safe-scan` + 零报告产出，替代 v1.4 Windows 条件 skip）（§17.B2'-R10） | R10；竞态窗口闭合意图升级为目录 inode 绑定 | R10 |
