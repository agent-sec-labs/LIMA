...
3. **错误分类（类别枚举冻结，stderr 单行，exit 2）**：`comp` 为 symlink → `ELOOP` → `target-invalid:symlink-component`；`comp` 非目录 → `ENOTDIR` → `target-invalid:not-a-directory`；`EACCES`/`EPERM` → `target-invalid:permission-denied`；`ENOENT` → `target-invalid:not-found`；其他 `OSError` → `target-invalid:io-error`（兜底）。类别枚举 = `{symlink-component, not-a-directory, permission-denied, not-found, io-error}`；stderr 允许回显调用方传入路径（参数错误口径不变），但不回显组件原文之外任何路径树信息。
...
5. **§17.B2' "目录本身为 symlink" 条款对齐**：末组件（target_dir 本身）为 symlink 同样由链上 `O_NOFOLLOW` → `ELOOP` → `target-invalid:symlink-component` 拒绝（旧 stderr 串 `error: target-dir must not be a symlink path` [v1.5-replaced] 废止，统一进 `target-invalid:` 枚举；非链接的 target-invalid 类别 exit 2 语义继承既有参数错误口径）。
