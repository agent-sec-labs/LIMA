...
REPAIR_PREVIEW = re.compile(r"^/v1/tasks/([0-9a-f-]+)/repair-preview$")
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    settings: Settings
    server_version = "LIMA/%s" % __version__
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if match:
                ...
                self._send_json(201, result)
                return
            match = REPAIR_PREVIEW.match(path)
            if match:
                principal = self._principal("fix")
                self._read_json(body)
                result = self.service.create_repair_preview(
                    match.group(1), principal.tenant_id
                )
                self.service.store.audit(
                    principal.tenant_id, principal.username, "repair.preview",
                    match.group(1), {
...
def run() -> None:
    ...
    print("LIMA dashboard: http://%s:%d" % (settings.host, settings.port))
