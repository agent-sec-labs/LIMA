"""Import isolation tests for the audit evidence package module (IP-0004 packet section 18.3)."""
...
AEP_MODULE = REPO_ROOT / "lima" / "contracts" / "aep.py"
...
class AepImportIsolationTests(unittest.TestCase):
    def test_module_public_api_matches_frozen_symbol_set(self):
        import lima.contracts.aep as aep

        self.assertEqual(frozenset(aep.__all__), FROZEN_MODULE_API)
        for name in sorted(FROZEN_MODULE_API):
            self.assertTrue(hasattr(aep, name), name)
        self.assertEqual(
            aep.AUDIT_EVIDENCE_PACKAGE_SCHEMA_NAME, "lima.audit-evidence-package"
        )

    def test_clean_process_import_has_no_db_network_docker_llm_service_or_legacy_models(self):
        script = (
            "import sys\n"
            "import lima.contracts.aep\n"
        ...
        self.assertIn("lima.contracts.aep", lima_modules)
    ...
    def test_module_only_uses_allowed_imports(self):
        ...
        source = AEP_MODULE.read_text(encoding="utf-8")
        roots: set[str] = set()
        ...
        self.assertFalse(
            unexplained,
            f"aep.py uses imports outside the frozen allowlist: {sorted(unexplained)}",
        )
    ...
    def test_import_does_not_change_lima_contracts_top_level_public_api(self):
        ...
        import lima.contracts.aep  # noqa: F401 -- import side effect under test
