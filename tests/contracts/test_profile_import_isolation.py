"""Import isolation tests for the repository profile module (IP-0003 packet section 18.3)."""
...
PROFILE_MODULE = REPO_ROOT / "lima" / "contracts" / "profile.py"
...
class ProfileImportIsolationTests(unittest.TestCase):
    def test_module_public_api_matches_frozen_symbol_set(self):
        import lima.contracts.profile as profile

        self.assertEqual(frozenset(profile.__all__), FROZEN_MODULE_API)
        for name in sorted(FROZEN_MODULE_API):
            self.assertTrue(hasattr(profile, name), name)
        self.assertEqual(
            profile.REPOSITORY_PROFILE_SCHEMA_NAME, "lima.repository-profile"
        )

    def test_clean_process_import_has_no_db_network_docker_llm_service_or_legacy_models(self):
        script = (
            "import sys\n"
            "import lima.contracts.profile\n"
        ...
        self.assertIn("lima.contracts.profile", lima_modules)
    ...
    def test_module_only_uses_allowed_imports(self):
        ...
        self.assertFalse(
            unexplained,
            f"profile.py uses imports outside the frozen allowlist: {sorted(unexplained)}",
        )
    ...
    def test_import_does_not_change_lima_contracts_top_level_public_api(self):
        ...
        import lima.contracts.profile  # noqa: F401 -- import side effect under test
