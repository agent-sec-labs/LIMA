"""Import isolation tests for the RVR module (IP-0006 packet section 18.3)."""
...
RVR_MODULE = REPO_ROOT / "lima" / "contracts" / "rvr.py"
...
class RvrImportIsolationTests(unittest.TestCase):
    def test_module_public_api_matches_frozen_symbol_set(self):
        import lima.contracts.rvr as rvr

        self.assertEqual(frozenset(rvr.__all__), FROZEN_MODULE_API)
        for name in sorted(FROZEN_MODULE_API):
            self.assertTrue(hasattr(rvr, name), name)
        self.assertEqual(
            rvr.REPAIR_VERIFICATION_REPORT_SCHEMA_NAME,
    ...
    def test_clean_process_import_has_no_db_network_docker_llm_service_or_legacy_models(self):
        ...
        self.assertIn("lima.contracts.rvr", lima_modules)
    ...
    def test_module_only_uses_allowed_imports(self):
        ...
        source = RVR_MODULE.read_text(encoding="utf-8")
        roots: set[str] = set()
        ...
        self.assertFalse(
            unexplained,
            f"rvr.py uses imports outside the frozen allowlist: {sorted(unexplained)}",
        )
    ...
    def test_import_does_not_change_lima_contracts_top_level_public_api(self):
        ...
        import lima.contracts.rvr  # noqa: F401 -- import side effect under test
