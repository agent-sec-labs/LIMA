"""Import isolation tests for the summary module (IP-0009 packet section 18.3)."""
...
SUMMARY_MODULE = REPO_ROOT / "lima" / "contracts" / "summary.py"
...
class SummaryImportIsolationTests(unittest.TestCase):
    def test_module_public_api_matches_frozen_symbol_set(self):
        import lima.contracts.summary as summary

        self.assertEqual(frozenset(summary.__all__), FROZEN_MODULE_API)
        for name in sorted(FROZEN_MODULE_API):
            self.assertTrue(hasattr(summary, name), name)
        self.assertEqual(summary.WORKFLOW_SUMMARY_SCHEMA_NAME, "lima.workflow-summary")
        self.assertEqual(summary.FAILURE_REPORT_SCHEMA_NAME, "lima.failure-report")

    def test_clean_process_import_has_no_db_network_docker_llm_service_or_legacy_models(self):
        script = (
            "import sys\n"
            "import lima.contracts.summary\n"
        ...
        self.assertIn("lima.contracts.summary", lima_modules)
        ...
        self.assertNotIn("lima.contracts.execution", lima_modules)
    ...
    def test_module_only_uses_allowed_imports(self):
        ...
        source = SUMMARY_MODULE.read_text(encoding="utf-8")
        roots: set[str] = set()
        ...
        self.assertFalse(
            unexplained,
            f"summary.py uses imports outside the frozen allowlist: {sorted(unexplained)}",
        )
    ...
    def test_import_does_not_change_lima_contracts_top_level_public_api(self):
        ...
        import lima.contracts.summary  # noqa: F401 -- import side effect under test
