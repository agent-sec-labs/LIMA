...
# v1 mock-based internal-failure case uses unittest.mock.patch.
...
class PortSanitizeTests(unittest.TestCase):
    def test_success_within_sink_kinds(self):
        ...
        for kind in ("storage", "log", "prompt", "api", "export"):
            with self.subTest(kind=kind):
                ...
                self.assertTrue(sp.sink_kind == kind and sp.tenant_id == "tenant-a")
    ...
    def test_allowlist_shrink_enforced(self):
    ...
    def test_missing_tenant_key(self):
        ...
        for bad in (b"", None):
            with self.subTest(bad=bad):
                ...
                self.assertEqual(ei.exception.code, PrivacyErrorCode.MISSING_TENANT_KEY)
    ...
    def test_invalid_argument_types(self):
        ...
        self.assertEqual(ei.exception.code, PrivacyErrorCode.INVALID_FIELD_TYPE)
        with self.assertRaises(PrivacyError) as ei:
            sanitize_for_sink(
    ...
    def test_sensitive_positions_replaced_by_fingerprint_records(self):
        ...
        self.assertIsInstance(sp.redacted_value, dict)
        self.assertIsInstance(sp.redacted_value["api_key"], FingerprintRecord)
        self.assertEqual(sp.redacted_value["n"], 2)
        self.assertEqual(sp.manifest.classification, ArtifactClassification.SENSITIVE)

    def test_no_leak_in_result_repr_or_entries(self):
    ...
    def test_preview_policy_controls_fingerprint_preview(self):
        ...
        self.assertTrue(rec.preview is not None and rec.preview.endswith("\u2026"))
        self.assertEqual(len(rec.preview), 3)  # 2 chars + ellipsis
        sp_off = sanitize_for_sink(
            EvidencePayload(payload_kind="structured_json", value={"password": SECRET}),
            _sink(),
            _policy(),
        )
        self.assertIsNone(sp_off.manifest.entries[0].preview)

    def test_policy_error_surface(self):
        ...
        self.assertEqual(ei.exception.code, PrivacyErrorCode.POLICY_ERROR)

    def test_internal_failure_wrapped(self):
        ...
        with mock.patch.object(port_mod, "classify_payload", boom):
            with self.assertRaises(PrivacyError) as ei:
                sanitize_for_sink(
                    EvidencePayload(payload_kind="text", value="x" * 8),
                    _sink(),
    ...
    def test_failure_returns_no_value(self):
    ...
    def test_deterministic_output_same_inputs(self):
        ...
        self.assertEqual(a.manifest.entries, b.manifest.entries)
        self.assertEqual(
            a.redacted_value["api_key"].fingerprint,
            b.redacted_value["api_key"].fingerprint,
        )

    def test_tenant_isolation_through_port(self):
    ...
    def test_tenant_key_never_in_output(self):
        ...
        self.assertNotIn(KEY, repr(sp).encode("utf-8", "backslashreplace"))
        self.assertNotIn(KEY.hex(), repr(sp))
