...
class AuditScriptSubprocessTests(unittest.TestCase):
    ...
    def test_script_fingerprints_differ_across_runs(self) -> None:
        # D3' §18 #6 (negative, RED on b4c4c06): each run uses an independent
    ...
    def test_script_oversize_file_identifiably_skipped(self) -> None:
        # F5' §17.F5' (negative, RED on b4c4c06): a file over
...
class DirFdBindingTests(unittest.TestCase):
    ...
    def test_source_uses_dirfd_binding_and_seam(self) -> None:
        # R10 (RED on b4c4c06): every per-file open is dir_fd-relative; no
    ...
    def test_directory_replacement_race_isolated(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as scan_tmp, tempfile.TemporaryDirectory() as out_tmp:
            ...
            (target / inside_1).write_text("bearer " + sentinel_1 + "\n", encoding="utf-8")
            (target / inside_2).write_text(json.dumps({"token": sentinel_2}), encoding="utf-8")
            ...
            fingerprints = [
                f for f in collect_key_values(result, "fingerprint") if isinstance(f, str)
            ]
            self.assertGreaterEqual(
                len(fingerprints),
                2,
                "zero-read or partial-read implementation: no findings for S1/S2",
            )
            self.assertGreaterEqual(
...
class SpecialFileTests(unittest.TestCase):
    ...
    def test_source_locks_nonblock_and_isreg(self) -> None:
        # R11 (RED on b4c4c06): O_NONBLOCK at the single openat site and a
...
class ScriptReadBudgetTests(unittest.TestCase):
    ...
    def test_source_has_no_full_read_bytes(self) -> None:
        # R2 (RED on b4c4c06 scripts/audit_sensitive_artifacts.py:67): the
    ...
    def test_file_budget_truncation_bounded_output(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = json.loads(result.stdout)
            self.assertEqual(report.get("status"), "incomplete")
            # O-5 revision: DIRECT selection-set size assertion -- the
            # selection set is exactly the 10,000 kept artifacts (no longer
            # only the indirect proxy of empty findings + truncated-at).
            self.assertEqual(report.get("artifact_count"), 10_000)
    ...
    def test_byte_budget_exceeded_report_still_written(self) -> None:
        # R2 (RED on b4c4c06): cumulative handle-read bytes past
...
class SecureReadVerificationTests(unittest.TestCase):
    ...
    def test_source_uses_open_verify_mechanism(self) -> None:
        # R3 (RED on b4c4c06): the script must use os.open + os.fstat and
    ...
    def test_identity_mismatch_fails_closed(self) -> None:
        # R3 (RED on b4c4c06): a mismatch between fstat(fd) and the
...
class SourcePathWhitelistTests(unittest.TestCase):
    ...
    def test_invalid_source_paths_rejected_without_echo(self) -> None:
        # R4 (RED on b4c4c06: no entry validation exists): paths, drive
    ...
    def test_invalid_source_path_rejected_structured_and_report(self) -> None:
        ...
        # artifact keys accepted by build_audit_report. O-2 revision: the
        # build_audit_report face is now ASSERTED here (previously only
        # named), completing the §18 #20 three-function coverage
        # (audit_text / audit_structured / build_audit_report).
        with self.assertRaises(PrivacyError) as ctx:
        ...
        # build_audit_report artifact-key face: an invalid artifact key in
        # the findings mapping is rejected with INVALID_FIELD_VALUE at
        # field_path "source_path" (D2' context-first signature).
        try:
            build_audit_report(make_context(), {"sk_live_ABC123xyztoken": ()}, POLICY)
        except TypeError:
            self.fail(
                "build_audit_report does not accept the D2' (context, findings, policy) "
                "signature yet (§17.D2' signature revision not implemented)"
            )
        except PrivacyError as exc:
            self.assertIs(exc.code, PrivacyErrorCode.INVALID_FIELD_VALUE)
            self.assertEqual(exc.field_path, "source_path")
            self.assertNotIn("sk_live_ABC123xyztoken", str(exc))
        else:
            self.fail("invalid artifact key accepted by build_audit_report (R4 §17.D1'.3)")
...
class TenantTagFormulaTests(unittest.TestCase):
    ...
    def test_packet_doc_pv_tmp_references_non_authoritative_only(self) -> None:
        # R5.4 docs hygiene, O-1 revision (contextual judgment): any residual
        # .pv_tmp reference inside the IP-0020 Packet must be non-authoritative
        # -- EITHER mechanically marked ("过程性证据"/"非权威") OR contextually
        # disavowed. Contextual allowance: version-history narration lines that
        # describe the act of replacing/abolishing a .pv_tmp authority
        # reference (e.g. the v1.3 header record "`.pv_tmp` 权威引用替换为
        # §17 自含 + Ledger 锚点") declare the disavowal by their own
        # semantics; a purely mechanical marker requirement was unsatisfiable
        # against frozen version-history lines (Coordinator review 2026-09-13
        # §六 O-1). Disavowal verbs are frozen: 替换/废止/删除/弃权.
        ...
        disavowal_verbs = ("替换", "废止", "删除", "弃权")
        for line in packet.read_text(encoding="utf-8").splitlines():
            if ".pv_tmp" not in line:
                continue
            marked = "过程性证据" in line or "非权威" in line
            contextually_disavowed = any(verb in line for verb in disavowal_verbs)
            self.assertTrue(
                marked or contextually_disavowed,
                f".pv_tmp reference neither marked nor contextually disavowed: {line[:80]}",
            )
...
class SameHandleReadTests(unittest.TestCase):
    ...
    def test_source_open_forms_whitelisted(self) -> None:
        # R7 (RED on b4c4c06): every reading-related open( in the script
        # must be an os.open( or os.fdopen( form -- no builtins
        # open(path, "rb"), no re-open after verification, no read_bytes(),
        # no st_size precheck. O-3 revision: the whitelist now judges the
        # CALL FORM (only os.open( / os.fdopen( pass); the previous 3-char
        # prefix tuple ("os.", "fd.") rejected compliant os.fdopen( calls
        # (the three characters preceding open( there are ".fd").
        source = SCRIPT_PATH.read_text(encoding="utf-8")
        self.assertNotIn("read_bytes", source)
        self.assertNotIn("st_size", source)
        ...
        for match in re.finditer(r"open\(", source):
            ...
            self.assertTrue(
                before.endswith("os.") or before.endswith("os.fd"),
                f"non-whitelisted open( call form at offset {match.start()}",
            )

    def test_single_os_open_per_file_zero_builtin_open(self) -> None:
        # R7 (RED on b4c4c06): wrapping builtins.open and os.open with
...
class OpenatChainTests(unittest.TestCase):
    ...
    def test_source_locks_component_openat_chain(self) -> None:
        # R13 (RED on b4c4c06 -- main opens the target by path, single-shot):
    ...
    def test_anchor_and_error_category_variants(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp, tempfile.TemporaryDirectory() as ext:
            ...
            (target / "s2.txt").write_text("bearer " + sentinel + "\n", encoding="utf-8")
...
class EnotdirDiagnosticMechanismLockTests(unittest.TestCase):
    """Packet v1.7 / Maintainer R17 §17.F5'-R13.3: the ENOTDIR diagnostic is
    exactly ONE os.stat(component, dir_fd=parent_fd, follow_symlinks=False)
    -- single stat, no open retry after it, no content read, fail-closed on
    every branch. Dual lock: source grep (form) + in-process monkeypatch
    counters (behavior), mirroring the SameHandleReadTests pattern."""
    ...
    def test_source_locks_enotdir_diagnostic_form(self) -> None:
        # R17 grep lock (all platforms): the script contains the diagnostic
        # os.stat call in its frozen form (dir_fd-bound component stat with
        # follow_symlinks=False) and NEVER stats with follow_symlinks=True
        # anywhere in the scan path (DirEntry.stat faces must equally pass
        # False -- no symlink following).
        source = SCRIPT_PATH.read_text(encoding="utf-8")
        self.assertRegex(source, r"os\.stat\([^()]*dir_fd\s*=")
        self.assertRegex(source, r"os\.stat\([^()]*follow_symlinks\s*=\s*False")
        self.assertNotIn("follow_symlinks=True", source)

    def _run_main_with_mechanism_counters(self, module, argv: list[str]):
        # R17 behavior-lock harness: wrap os.stat / os.open / os.fdopen /
        # builtins.open with counters around one in-process module.main run.
        # The diagnostic discriminator is the follow_symlinks=False kwarg
        # (pathlib/os.path internal stats never pass False), so incidental
        # parameter-validation stats are not miscounted as diagnostics.
        import builtins as _builtins
        import contextlib
        import io as _io
        ...
        counters = {
            "os_open": 0,
            "os_open_at_diagnostic": None,
            "stat_follow_false": 0,
            "os_fdopen": 0,
            "builtin_open": 0,
        }
        real_os_open, real_os_stat = os.open, os.stat
        real_os_fdopen, real_builtin_open = os.fdopen, open

        def counting_os_open(*args, **kwargs):
            counters["os_open"] += 1
            return real_os_open(*args, **kwargs)

        def counting_os_stat(*args, **kwargs):
            if kwargs.get("follow_symlinks") is False:
                counters["stat_follow_false"] += 1
                counters["os_open_at_diagnostic"] = counters["os_open"]
            return real_os_stat(*args, **kwargs)

        def counting_os_fdopen(*args, **kwargs):
            counters["os_fdopen"] += 1
            return real_os_fdopen(*args, **kwargs)
    ...
    def test_not_a_directory_diagnostic_single_stat_no_retry_no_read(self) -> None:
        # R17 behavior lock (POSIX; Windows covered by the R10 probe branch
        # below): a plain FILE as the final target component under combined
        # O_DIRECTORY|O_NOFOLLOW flags is the DETERMINISTIC ENOTDIR case on
        # every POSIX platform. Contract §17.F5'-R13.3 v1.7: the diagnostic
        # is one os.stat(..., follow_symlinks=False); after it the run
        # classifies (not-a-directory) and fails closed -- no open retry
        # (os.open count frozen at the diagnostic moment), no content read
        # (zero os.fdopen / zero builtins.open), single stderr line, exit 2.
        module = self._load_script_module()
        with tempfile.TemporaryDirectory() as tmp:
            target = pathlib.Path(tmp)
            plain_file = target / "plain.txt"
            plain_file.write_text("bearer " + KNOWN_SECRET_VALUES[0] + "\n", encoding="utf-8")
            code, stdout, stderr, counters = self._run_main_with_mechanism_counters(
                module, [str(plain_file)]
            )
            if not posix_main_or_fail_closed(self, code, stderr):
                return
            self.assertEqual(code, 2, stderr)
            self.assertIn("target-invalid:not-a-directory", stderr)
            stderr_lines = [ln for ln in stderr.splitlines() if ln.strip()]
            self.assertEqual(len(stderr_lines), 1, stderr)
            self.assertEqual(stdout.strip(), "", stdout)
            # Exactly one diagnostic stat (follow_symlinks=False), and the
            # diagnostic actually fired (moment recorded).
            self.assertEqual(counters["stat_follow_false"], 1, counters)
            self.assertIsNotNone(counters["os_open_at_diagnostic"], counters)
            # No open retry: the os.open count did not grow after the
            # diagnostic fired.
            self.assertEqual(counters["os_open"], counters["os_open_at_diagnostic"], counters)
            # No content read on any branch of the failed startup.
            self.assertEqual(counters["os_fdopen"], 0, counters)
            self.assertEqual(counters["builtin_open"], 0, counters)
            self.assertNotIn(KNOWN_SECRET_VALUES[0], stdout + stderr)

    def test_symlink_component_diagnostic_bounded_fail_closed(self) -> None:
        # R17 behavior lock, errno-agnostic branch: for a symlink component
        # the platform may return ELOOP (no diagnostic needed, 0 stats) or
        # ENOTDIR (exactly one diagnostic stat) -- either way the diagnostic
        # count never exceeds one, no open retry happens after a diagnostic,
        # nothing is read, and the category is symlink-component with a
        # single stderr line and exit 2 (inside-b sentinel unread/absent).
        module = self._load_script_module()
        if IS_WINDOWS_PLATFORM:
            # R10 platform probe fires before the component chain on
            # Windows: fail-closed shape only (mechanism not exercisable).
            with tempfile.TemporaryDirectory() as tmp:
                target = pathlib.Path(tmp)
                (target / "s.txt").write_text("plain\n", encoding="utf-8")
                result = run_script(str(target))
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(code, 2, stderr)
            self.assertIn("target-invalid:symlink-component", stderr)
            stderr_lines = [ln for ln in stderr.splitlines() if ln.strip()]
            self.assertEqual(len(stderr_lines), 1, stderr)
            self.assertEqual(stdout.strip(), "", stdout)
            self.assertLessEqual(counters["stat_follow_false"], 1, counters)
            if counters["stat_follow_false"] == 1:
                # ENOTDIR branch: no open retry after the one diagnostic.
                self.assertEqual(counters["os_open"], counters["os_open_at_diagnostic"], counters)
            self.assertEqual(counters["os_fdopen"], 0, counters)
            self.assertEqual(counters["builtin_open"], 0, counters)
...
class SuffixPreFilterTests(unittest.TestCase):
    ...
    def test_unsupported_suffixes_do_not_consume_quota(self) -> None:
        # R14 (RED on b4c4c06 -- main counts every directory entry toward
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (target / "z-sentinel.txt").write_text("bearer " + sentinel + "\n", encoding="utf-8")
            result = run_script(str(target), timeout=600)
            if not posix_script_or_fail_closed(self, result):
                return
            self.assertEqual(result.returncode, 0, result.stderr)
...
class FindingsBudgetTests(unittest.TestCase):
    ...
    def test_findings_capped_at_report_budget(self) -> None:
        # R15 (RED on b4c4c06 -- build_audit_report appends unboundedly):
        ...
        # §17.F5'-R15.1 (same family as max_files_per_scan); RED on b4c4c06
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name in ("f0.json", "f1.json", "f2.json"):
                (target / name).write_text(json.dumps(members), encoding="utf-8")
            result = run_script(str(target), timeout=600)
            if not posix_script_or_fail_closed(self, result):
                return
            self.assertEqual(result.returncode, 3, result.stderr)
