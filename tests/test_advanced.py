...
from lima.evolution import EvolutionEngine, RegressionEvaluator
...
class AdvancedFeatureTests(unittest.TestCase):
    ...
    def test_safe_fixer_changes_only_supported_rules(self):
        ...
        self.assertEqual("os.environ", ast.unparse(assignment.value.value))
        ...
        self.assertEqual(
            [{"rule_id": "SEC-EVAL", "line": 2, "reason": "unsupported-rule"}],
            result["blocked"],
        )

    def test_safe_fixer_blocks_unverified_supported_finding(self):
        content = "import subprocess\nsubprocess.run('echo ' + command, shell=True)\n"
        candidate = {
            "path": "app.py", "line": 2, "rule_id": "SEC-SUBPROCESS-SHELL",
            "verification_state": "candidate",
        }
        verified = {**candidate, "verification_state": "dataflow-verified"}

        blocked = SafeFixer().apply(content, [candidate], "app.py")
        repaired = SafeFixer().apply(content, [verified], "app.py")

        self.assertEqual([], blocked["rules"])
        self.assertIn("shell=True", blocked["content"])
        self.assertEqual("requires-dataflow-verification", blocked["blocked"][0]["reason"])
        self.assertEqual(["SEC-SUBPROCESS-SHELL"], repaired["rules"])
        self.assertIn("shell=False", repaired["content"])
        self.assertIn("['echo', command]", repaired["content"])

    def test_fix_publication_does_not_read_files_for_unverified_findings(self):
        class Client:
            ...
            def get_file(self, *_args):
                raise AssertionError("unverified findings must not trigger file reads")

        result = SafeFixer().create_fix_commits(
            Client(), "org/repo", 7,
            {"findings": [{
                "path": "app.py", "line": 1,
        ...
        self.assertEqual(
            "requires-dataflow-verification", result["blocked_findings"][0]["reason"]
        )
